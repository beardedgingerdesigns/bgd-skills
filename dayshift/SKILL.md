...
7. **Workers report; you verify; then you close. Don't poll.** Never continuously ping a running agent for a status update — let it work. A worker reports back to you when it is finished. Only then do you verify its output (a different vendor than authored it — see Routing). **Only on true verification** do you tear down that worker's pane. If the pane's tab holds other live agents, leave the tab until every agent in it is verified and closed — never close a tab out from under a live agent. Only close panes and tabs this shift created; target by pane/tab id; stay in your own workspace. Left-open dead panes are the sprawl that turns one lane into five.
